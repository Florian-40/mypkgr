...
output: 
  rmarkdown::html_vignette: 
    toc: true
vignette: >
  %\VignetteIndexEntry{execution_time}
...
autoplot(mb, expression, breaks=c(0.1,1), minor_breaks=c(seq(0,0.1,0.01),seq(0.1,1,0.1)))
```
...
```{r}
Rcpp::sourceCpp("/Users/florianrobert/R/mypkgr/src/mvnpdfC.cpp")
n <- 1000
mb <- microbenchmark(mvtnorm::dmvnorm(matrix(1.96, nrow = n, ncol = 2)),
                     mvnpdf(x=matrix(1.96, nrow = 2, ncol = n), Log=FALSE),
                     mvnpdfsmart(x=matrix(1.96, nrow = 2, ncol = n), Log=FALSE),
...
autoplot(mb, expression, breaks=c(0.1,1, 10), minor_breaks=c(seq(0,0.1,0.01),seq(0.1,1,0.1), seq(1,10,1)))
```
